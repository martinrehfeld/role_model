module RoleModel
  module ClassMethods
    ...
    def mask_for(*roles)
      sanitized_roles = roles.map { |role| role.respond_to?(:each) ? role.to_a : role }.flatten.map { |role| role.to_sym } 
      (sanitized_roles & valid_roles).map { |role| 2**valid_roles.index(role) }.inject { |sum, bitvalue|
        sum + bitvalue
      } || 0
    end
